...
// Credit https://www.habrador.com/tutorials/select-units-within-rectangle/ (Need to investigate)
namespace WaS
{
    public class SelectionSquare : MonoBehaviour
    {
        ...
        void SelectUnits()
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (Physics.Raycast(pCam.ScreenPointToRay(Input.mousePosition), out hit, 200f, 1 << 8))
                {
            ...
            if (isClicking)
            {
                ...
                RaycastHit hit;
                //Fire ray from camera
                if (Physics.Raycast(pCam.ScreenPointToRay(Input.mousePosition), out hit, 200f))
                {
        ...
        void DisplaySquare()
        {
            ...
            Vector3 squareStartScreen = pCam.WorldToScreenPoint(squareStartPos);
            ...
            int i = 0;
            //Fire ray from camera
            if (Physics.Raycast(pCam.ScreenPointToRay(TL), out hit, 200f, 1 << 8))
            {
                TL = hit.point;
                i++;
            }
            if (Physics.Raycast(pCam.ScreenPointToRay(TR), out hit, 200f, 1 << 8))
            {
                TR = hit.point;
                i++;
            }
            if (Physics.Raycast(pCam.ScreenPointToRay(BL), out hit, 200f, 1 << 8))
            {
                BL = hit.point;
                i++;
            }
            if (Physics.Raycast(pCam.ScreenPointToRay(BR), out hit, 200f, 1 << 8))
            {
                BR = hit.point;
                i++;
            }
